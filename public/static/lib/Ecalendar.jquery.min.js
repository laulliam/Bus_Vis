$.fn.ECalendar=function(config){
    ...
    var _this=$(this),ECalendar,day,imonth,iday,iyear,ilast,itimebox,select,
        getFullYear,getMonth,getDate,weekday,getHour,getMinutes,oper,
        currenttime,x,y,mx,my,timestep=config.step,drag,input,dragnum,isinit=false,
    ...
    config.type=config.type=="time"?0:1;callback=config.callback;
    if(!IsPC()) {
        if(config.type==0) {
    ...
    config.skin=tmp[config.skin]?tmp[config.skin]:config.skin;init();
    _this.click(function(event){
        var itime=_this.attr("data-ec")?_this.attr("data-ec"):_this.val();
        var date=itime?new Date(itime.replace(/-/g,"/")):con.date;
    ...
    day.on("mousedown","li",function(e){
        ...
        if(config.type==0)
        {
            ...
            // drag.attr("data-settime",getHour<10?("0"+getHour+":00"):(getHour+":00"));
            var a=getHour*30;
    ...
    $(imonth).mousedown(function(e){
        ...
        var prehtml='<i class="r3" data="'+(dragnum-3)+'">'+(dragnum-3)+'</i>'+'<i class="r2" data="'+(dragnum-2)+'">'+
            (dragnum-2)+'</i>'+'<i class="r1" data="'+(dragnum-1)+'">'+(dragnum-1)+'</i>';
        var apphtml='<i class="a1" data="'+(dragnum+1)+'">'+(dragnum+1)+'</i>'+'<i class="a2" data="'+(dragnum+2)+'">'+
            (dragnum+2)+'</i>'+'<i class="a3" data="'+(dragnum+3)+'">'+(dragnum+3)+'</i>';
    ...
    function init() {
        isinit=true;
        var html='<div class="ECalendarBox ECalendarNoneSelect" style="display:none"><input type="hidden"/><div class="head">' +
            '<div class="currentdate"><h2><span class="iyear"><span class="activ"></span></span>/<span class="imonth">' +
            '<span class="activ"></span></span></h2><ul class="oper"><li>&lt;</li><li>&gt;</li></ul></div><div class="currenttime">' +
            '<span class="ilasttext"></span><span class="itime itimebox"></span></div><ul class="select">' +
            '</ul><ul class="week">' +
            '<li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li>六</li><li>日</li></ul></div><ul class="day"></ul></div></div>';
        ECalendar=$(html);
        ECalendar.css("border-color",config.skin);
        ECalendar.children('div.head').css({"border-color":"#302b63","background":"#302b63"});
        day=ECalendar.find("ul.day");imonth=ECalendar.find("span.imonth");
        iday=ECalendar.find("span.iday");ilast=ECalendar.find("span.ilasttext");
        itimebox=ECalendar.find("span.itimebox");iyear=ECalendar.find("span.iyear");
        select=ECalendar.find("ul.select");oper=ECalendar.find("ul.oper li");
        currenttime=ECalendar.find("div.currenttime,ul.week");
        input=ECalendar.children("input[type='hidden']");
        var name=_this.attr("name");_this.removeAttr("name");
        input.attr("name",name);
        _this.after('<svg t="1493098552635" class="icon" style="position: absolute;right:'+(_this.height()-20)/2+'px;' +
            'top:50%;margin-top: -10px;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1006" ' +
            'xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><defs><style type="text/css"></style></defs>' +
            '<path d="M856.576 200.192v54.784h58.368V814.08H109.568V256.512h56.832v-56.32H51.2V870.4h921.6V200.192h-116.224z" ' +
            'fill="#666666" p-id="1007"></path><path d="M819.2 102.4h-204.288v255.488L819.2 358.4V102.4z" fill="#EF4848" p-id="1008">' +
            '</path><path d="M563.712 664.064h256V716.8h-256zM563.712 512h256v51.2h-256zM461.312 200.192h102.4v56.32h-102.4zM205.312 ' +
            '664.064h256V716.8h-256zM205.312 512h256v51.2h-256z" fill="#666666" p-id="1009"></path><path d="M410.112 102.4H204.8v256l2' +
            '05.312-0.512V102.4z" fill="#EF4848" p-id="1010"></path></svg>');
            ...
            for(var i=1;i<=max;i++) {
                if(i<oneday)
                    ...
                    if((i-oneday+1)==getDate)
                        list+="<li class='activ' data='"+(i-oneday+1)+"' style='background:#302b63'>"+(i-oneday+1)+"</li>";
                    else
